...
\addbibresource{example,bib_game,bib_tech,bib_mcts,bib_moba,bib_mywork,bib_rl,bib_eomm,bib_dda,bib_churn} % The filename of the bibliography
...
\thesistitle{Recommendation Systems for Influencing Game Outcomes and Player Engagement in Pre-Match of Match-Based Games} % Your thesis title, this is used in the title and abstract, print it elsewhere with \ttitle
...
For years, video games have garnered the attention of people worldwide, providing a unique environment for entertainment, aesthetic appreciation, story adventure, knowledge acquirement, competition and socialization. There has been an extensive body of theories and empirical studies suggesting that game outcomes have a close relationship with player engagement. One plausible approach to influence game outcomes and player engagement is through dedicatedly crafting and presenting in-game elements to players. In-game elements, such as weapons, maps, levels, and characters, influence how players interact with a game and could directly modulate the outcome of a game (e.g., win, loss, level-up, and milestones unlocks) and player engagement. 
...
In this thesis I propose to use recommendation system techniques in order to recommend players in-game elements for influencing game outcomes and engagement. While there are numerous scenarios where in-game element recommendations are applicable, my thesis is delimited by the following boundaries: (1) the specific types of in-game elements for recommendations are starting items, characters and opponents; (2) the context of applying the recommendations is within the pre-match stage of match-based video games, when in-game elements determined before a match starts could already have an impact on the final match outcome. Within these boundaries, I propose three recommendation systems with each coping with different goals and challenges.
...
First, I propose a deck recommendation system named \textit{Q-DeckRec} for recommending winning-effective decks, a set of cards seen as the starting items, in one-vs-one Collectible Card Games (CCG). Q-DeckRec deals with the challenge of how to quickly identify a winning-effective deck from a large number of possible choices using minimal computational resources; thus Q-DeckRec is designed to be suitable for large-scale or real-time recommendation application.

Second, I propose a character recommendation system, \textit{DraftArtist}, for recommending winning-effective characters in Multi-player Online Battle Arena (MOBA) games. Matches in MOBA games are team-based and each player needs to select, in a specific order, a character  with unique abilities which should synergize the teammates' and counter the opponents'. The challenge that DraftArtist deals with is efficient planning on which characters other players will select and identify winning-effective characters for adapting to the ultimate character line-up, which could embrace a large number of possibilities and uncertainty in such a team-based competitive setting.

Third, I propose an opponent recommendation framework named\linebreak \textit{Engagement-Optimized Matchmaking} (EOMM), in which expected match outcomes within recommended opponents optimize all involved players' engagement. EOMM presents a new way of thinking opponent recommendation as an optimization problem which can be solved through a graph matching algorithm. 

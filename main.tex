...
\thesistitle{Pre-Match Recommendations for Influencing Match Outcomes and Player Engagement in Video Games} % Your thesis title, this is used in the title and abstract, print it elsewhere with \ttitle
...
Within these boundaries, I propose three recommendation systems with each coping with different challenges:
...
Second, I propose a character recommendation system, \textit{DraftArt}, for recommending winning-effective characters in Multi-player Online Battle Arena (MOBA) games. Matches in MOBA games are team-based. In the pre-match stage, each player needs to select a distinct character with unique abilities which should synergize the teammates' and counter the opponents'  in the match. As the character selection is progressive, the challenge that DraftArt deals with is to predict which characters will be selected by others, which embraces a large number of possibilities, and efficiently identify winning-effective characters which will fit the ultimate character line-up.

Third, I propose an opponent recommendation system, a matchmaking framework named \textit{EOMM}, in which expected match outcomes within recommended opponents optimize all involved players' engagement. While the aforementioned starting item and character recommendation systems aim for  increasing the win rate of one player, which can be seen as an intermediate goal to improve player engagement, the opponent recommendation system directly operate on the optimization of population-wise player engagement through opponent matching.

...
\subsection{Matchmaking Strategies}
...
%A summary of MWPM solution complexity in general graph is shown in Table 2.
%
%\begin{table}[]
%\centering
%\caption{Complexity of MWPM algorithms. $n=|V|, m=|E|$}
%\begin{tabular}{ll}
%\hline
%Algorithm & Complexity                      \\ \hline
%\cite{edmonds1965maximum,edmonds1965paths}         & $O(n^2 m)$         \\
%\begin{tabular}[x]{@{}c@{}}\cite{gabow1974implementation}\\ \cite{lawler2001combinatorial} \end{tabular}          & $O(n^3)$             \\
%\cite{karzanov1976efficient}         & $O(n^3 + m\log n)$      \\
%\cite{gabow1990data}         & $O(n(m+n\log n)$              \\
%\cite{gabow1985scaling}\tablefootnote{ Integer edge weights only. $K$ is the largest magnitude of an edge weight. } & $O(mn^{3/4}\log K)$  \\
%
%\cite{drake2003simple}\tablefootnote{Greedy algorithm with 1/2 performance ratio. } & $O(m)$  \\
%\cite{duan2014linear}\tablefootnote{Greedy algorithm with $(1-\epsilon)$ performance ratio.} & $O(m\epsilon^{-1}\log \epsilon^{-1})$
%\end{tabular}
%\end{table}
...
Besides generating optimal matchmaking assignments, EOMM provides a framework to conduct theoretical analysis on other matchmaking related problems. We use this framework to compare \linebreak EOMM with other matchmaking strategies under different hypothetical situations to obtain many insights. Without loss of generality, we focus our discussion on 1-vs-1 games with possible game outcomes sampled from \emph{Win}, \emph{Lose} and \emph{Draw}.
...
\item If $c(Win) + c(Lose)< 2\cdot c(Draw)$, equal-skill based matchmaking is actually \emph{worst} among all matchmaking \linebreak schemes, as its goal to create close matches contrarily minimizes the overall player engagement. Although this situation contradicts with the common intuition that fair matches are good, it is possible for a real game. Therefore validating the assumptions with real game data is critical before applying an equal-skill based matchmaking algorithm.
\end{itemize}
